#!/bin/bash
PATH=/var/lib/mongodb-mms-automation/bin:/sbin:/bin:/usr/sbin:/usr/bin
cd /backup/
SERVER=MONGODB-POE
SOURCEBACKUP=backup-$(date --date "-1 days" +'%Y-%m-%d')
DESTINATIONBACKUP=/backup/google/mongodb-poe
